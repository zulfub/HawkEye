...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("C:/tensorflow1/models/research/object_detection/img_proc/classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("C:/tensorflow1/models/research/object_detection/img_proc/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        if ProcessImg.showSteps == True: # show steps ###################################################
            ...
            cv2.drawContours(imgContours, contours, -1, ProcessImg.SCALAR_WHITE)
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        cv2.rectangle(imgThreshColor, pt1, pt2, ProcessImg.SCALAR_GREEN, 2)           # draw green box around the char
